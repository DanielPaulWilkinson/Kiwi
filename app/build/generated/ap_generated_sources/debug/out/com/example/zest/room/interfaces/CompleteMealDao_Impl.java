...
public final class CompleteMealDao_Impl extends CompleteMealDao {
  ...
  public CompleteMealDao_Impl(RoomDatabase __db) {
    this.__db = __db;
  ...
  @Override
  public CompleteMeal GetFullMealNonLive(final long id) {
    __db.beginTransaction();
    try {
      CompleteMeal _result = CompleteMealDao_Impl.super.GetFullMealNonLive(id);
      __db.setTransactionSuccessful();
      return _result;
    } finally {
      __db.endTransaction();
  ...
  @Override
  public LiveData<List<CompleteMeal>> GetAllCompleteMeal() {
    final String _sql = "select * from meal";
    final RoomSQLiteQuery _statement = RoomSQLiteQuery.acquire(_sql, 0);
    return __db.getInvalidationTracker().createLiveData(new String[]{"MealIngredientsJoin","Ingredients","MealMethodJoin","Method","meal"}, true, new Callable<List<CompleteMeal>>() {
      @Override
      public List<CompleteMeal> call() throws Exception {
        __db.beginTransaction();
        try {
          ...
          try {
            ...
            final int _cursorIndexOfTimeOfDay = CursorUtil.getColumnIndexOrThrow(_cursor, "timeOfDay");
            final int _cursorIndexOfComplexity = CursorUtil.getColumnIndexOrThrow(_cursor, "complexity");
            final LongSparseArray<ArrayList<Ingredients>> _collectionIngredients = new LongSparseArray<ArrayList<Ingredients>>();
            final LongSparseArray<ArrayList<Method>> _collectionMethod = new LongSparseArray<ArrayList<Method>>();
            while (_cursor.moveToNext()) {
              if (!_cursor.isNull(_cursorIndexOfMealId)) {
                final long _tmpKey = _cursor.getLong(_cursorIndexOfMealId);
                ArrayList<Ingredients> _tmpIngredientsCollection = _collectionIngredients.get(_tmpKey);
                if (_tmpIngredientsCollection == null) {
                  _tmpIngredientsCollection = new ArrayList<Ingredients>();
                  _collectionIngredients.put(_tmpKey, _tmpIngredientsCollection);
                }
              }
              if (!_cursor.isNull(_cursorIndexOfMealId)) {
                final long _tmpKey_1 = _cursor.getLong(_cursorIndexOfMealId);
                ArrayList<Method> _tmpMethodCollection = _collectionMethod.get(_tmpKey_1);
                if (_tmpMethodCollection == null) {
                  _tmpMethodCollection = new ArrayList<Method>();
                  _collectionMethod.put(_tmpKey_1, _tmpMethodCollection);
                }
              }
            }
            _cursor.moveToPosition(-1);
            __fetchRelationshipIngredientsAscomExampleZestRoomEntitiesIngredients(_collectionIngredients);
            __fetchRelationshipMethodAscomExampleZestRoomEntitiesMethod(_collectionMethod);
            final List<CompleteMeal> _result = new ArrayList<CompleteMeal>(_cursor.getCount());
            while(_cursor.moveToNext()) {
              final CompleteMeal _item;
              final Meal _tmpMeal;
              if (! (_cursor.isNull(_cursorIndexOfMealId) && _cursor.isNull(_cursorIndexOfTitle) && _cursor.isNull(_cursorIndexOfCheap) && _cursor.isNull(_cursorIndexOfHealthScore) && _cursor.isNull(_cursorIndexOfVegetarian) && _cursor.isNull(_cursorIndexOfVegan) && _cursor.isNull(_cursorIndexOfGlutenFree) && _cursor.isNull(_cursorIndexOfVeryHealthy) && _cursor.isNull(_cursorIndexOfDairyFree) && _cursor.isNull(_cursorIndexOfVeryPopular) && _cursor.isNull(_cursorIndexOfSustainable) && _cursor.isNull(_cursorIndexOfPricePerServing) && _cursor.isNull(_cursorIndexOfSpoonId) && _cursor.isNull(_cursorIndexOfServings) && _cursor.isNull(_cursorIndexOfSourceUrl) && _cursor.isNull(_cursorIndexOfSummary) && _cursor.isNull(_cursorIndexOfImage) && _cursor.isNull(_cursorIndexOfReadyInMinutes) && _cursor.isNull(_cursorIndexOfDiets) && _cursor.isNull(_cursorIndexOfDishTypes) && _cursor.isNull(_cursorIndexOfOccasions) && _cursor.isNull(_cursorIndexOfLikes) && _cursor.isNull(_cursorIndexOfUsedIngredientCount) && _cursor.isNull(_cursorIndexOfCuisines) && _cursor.isNull(_cursorIndexOfTimeOfDay) && _cursor.isNull(_cursorIndexOfComplexity))) {
                final long _tmpMealId;
                _tmpMealId = _cursor.getLong(_cursorIndexOfMealId);
                final String _tmpTitle;
                _tmpTitle = _cursor.getString(_cursorIndexOfTitle);
                ...
                _tmp_9 = _cursor.getInt(_cursorIndexOfComplexity);
                _tmpComplexity = Converters.fromIntToComplexity(_tmp_9);
                _tmpMeal = new Meal(_tmpMealId,_tmpSpoonId,_tmpTitle,_tmpSummary,_tmpHealthScore,_tmpPricePerServing,_tmpCheap,_tmpSourceUrl,_tmpVegetarian,_tmpVegan,_tmpGlutenFree,_tmpVeryHealthy,_tmpDairyFree,_tmpVeryPopular,_tmpSustainable,_tmpDiets,_tmpServings,_tmpReadyInMinutes,_tmpImage,_tmpDishTypes,_tmpOccasions,_tmpLikes,_tmpUsedIngredientCount,_tmpTimeOfDay,_tmpComplexity);
                _tmpMeal.cuisines = _cursor.getString(_cursorIndexOfCuisines);
              }  else  {
                _tmpMeal = null;
              }
              ArrayList<Ingredients> _tmpIngredientsCollection_1 = null;
              if (!_cursor.isNull(_cursorIndexOfMealId)) {
                final long _tmpKey_2 = _cursor.getLong(_cursorIndexOfMealId);
                _tmpIngredientsCollection_1 = _collectionIngredients.get(_tmpKey_2);
              }
              if (_tmpIngredientsCollection_1 == null) {
                _tmpIngredientsCollection_1 = new ArrayList<Ingredients>();
              }
              ArrayList<Method> _tmpMethodCollection_1 = null;
              if (!_cursor.isNull(_cursorIndexOfMealId)) {
                final long _tmpKey_3 = _cursor.getLong(_cursorIndexOfMealId);
                _tmpMethodCollection_1 = _collectionMethod.get(_tmpKey_3);
              }
              if (_tmpMethodCollection_1 == null) {
                _tmpMethodCollection_1 = new ArrayList<Method>();
              }
              _item = new CompleteMeal();
              _item.meal = _tmpMeal;
              _item.ingredients = _tmpIngredientsCollection_1;
              _item.method = _tmpMethodCollection_1;
              _result.add(_item);
            }
            __db.setTransactionSuccessful();
            return _result;
  ...
  @Override
  public LiveData<CompleteMeal> GetCompleteMeal(final long id) {
    final String _sql = "select * from meal WHERE mealId=? ";
    final RoomSQLiteQuery _statement = RoomSQLiteQuery.acquire(_sql, 1);
    int _argIndex = 1;
    _statement.bindLong(_argIndex, id);
    return __db.getInvalidationTracker().createLiveData(new String[]{"MealIngredientsJoin","Ingredients","MealMethodJoin","Method","meal"}, true, new Callable<CompleteMeal>() {
      @Override
      public CompleteMeal call() throws Exception {
        __db.beginTransaction();
        try {
          final Cursor _cursor = DBUtil.query(__db, _statement, true, null);
          try {
            ...
            if(_cursor.moveToFirst()) {
              ...
              _result = new CompleteMeal();
              _result.meal = _tmpMeal;
              _result.ingredients = _tmpIngredientsCollection_1;
              _result.method = _tmpMethodCollection_1;
            } else {
              _result = null;
            }
            __db.setTransactionSuccessful();
  ...
  @Override
  public CompleteMeal GetCompleteMealNonLive(final long id) {
    final String _sql = "select * from meal WHERE mealId=? ";
    final RoomSQLiteQuery _statement = RoomSQLiteQuery.acquire(_sql, 1);
    int _argIndex = 1;
    _statement.bindLong(_argIndex, id);
  ...
  @Override
  public CompleteMeal CheckExistsBySpoonId(final Long spoonId) {
    final String _sql = "select * from meal WHERE spoonId=? ";
    final RoomSQLiteQuery _statement = RoomSQLiteQuery.acquire(_sql, 1);
    int _argIndex = 1;
    if (spoonId == null) {
      _statement.bindNull(_argIndex);
    } else {
      _statement.bindLong(_argIndex, spoonId);
    }
    __db.assertNotSuspendingTransaction();
    __db.beginTransaction();
    try {
  ...
  private void __fetchRelationshipIngredientsAscomExampleZestRoomEntitiesIngredients(
      final LongSparseArray<ArrayList<Ingredients>> _map) {
    if (_map.isEmpty()) {
      return;
    }
    // check if the size is too big, if so divide;
    if(_map.size() > RoomDatabase.MAX_BIND_PARAMETER_CNT) {
      LongSparseArray<ArrayList<Ingredients>> _tmpInnerMap = new LongSparseArray<ArrayList<Ingredients>>(androidx.room.RoomDatabase.MAX_BIND_PARAMETER_CNT);
      int _tmpIndex = 0;
      int _mapIndex = 0;
      ...
      while(_mapIndex < _limit) {
        ...
        if(_tmpIndex == RoomDatabase.MAX_BIND_PARAMETER_CNT) {
          __fetchRelationshipIngredientsAscomExampleZestRoomEntitiesIngredients(_tmpInnerMap);
          _tmpInnerMap = new LongSparseArray<ArrayList<Ingredients>>(RoomDatabase.MAX_BIND_PARAMETER_CNT);
          _tmpIndex = 0;
        }
      }
      if(_tmpIndex > 0) {
    ...
    _stringBuilder.append("SELECT `Ingredients`.`ingredientsId` AS `ingredientsId`,`Ingredients`.`spoonId` AS `spoonId`,`Ingredients`.`name` AS `name`,`Ingredients`.`description` AS `description`,`Ingredients`.`brand` AS `brand`,`Ingredients`.`aisle` AS `aisle`,`Ingredients`.`nameClean` AS `nameClean`,`Ingredients`.`original` AS `original`,`Ingredients`.`consistency` AS `consistency`,`Ingredients`.`freezable` AS `freezable`,`Ingredients`.`microwaveable` AS `microwaveable`,`Ingredients`.`vegan` AS `vegan`,`Ingredients`.`glutenFree` AS `glutenFree`,`Ingredients`.`servingSize` AS `servingSize`,`Ingredients`.`Units` AS `Units`,`Ingredients`.`energy` AS `energy`,`Ingredients`.`fat` AS `fat`,`Ingredients`.`ofWhichSaturates` AS `ofWhichSaturates`,`Ingredients`.`fiber` AS `fiber`,`Ingredients`.`protein` AS `protein`,`Ingredients`.`salt` AS `salt`,`Ingredients`.`calories` AS `calories`,`Ingredients`.`sugar` AS `sugar`,_junction.`mealId` FROM `MealIngredientsJoin` AS _junction INNER JOIN `Ingredients` ON (_junction.`ingredientsId` = `Ingredients`.`ingredientsId`) WHERE _junction.`mealId` IN (");
    ...
    try {
      final int _itemKeyIndex = 23; // _junction.mealId;
      if (_itemKeyIndex == -1) {
        return;
      }
      final int _cursorIndexOfIngredientsId = CursorUtil.getColumnIndex(_cursor, "ingredientsId");
      final int _cursorIndexOfSpoonId = CursorUtil.getColumnIndex(_cursor, "spoonId");
      ...
      final int _cursorIndexOfBrand = CursorUtil.getColumnIndex(_cursor, "brand");
      final int _cursorIndexOfAisle = CursorUtil.getColumnIndex(_cursor, "aisle");
      final int _cursorIndexOfNameClean = CursorUtil.getColumnIndex(_cursor, "nameClean");
      final int _cursorIndexOfOriginal = CursorUtil.getColumnIndex(_cursor, "original");
      final int _cursorIndexOfConsistency = CursorUtil.getColumnIndex(_cursor, "consistency");
      final int _cursorIndexOfFreezable = CursorUtil.getColumnIndex(_cursor, "freezable");
      final int _cursorIndexOfMicrowaveable = CursorUtil.getColumnIndex(_cursor, "microwaveable");
      final int _cursorIndexOfVegan = CursorUtil.getColumnIndex(_cursor, "vegan");
      final int _cursorIndexOfGlutenFree = CursorUtil.getColumnIndex(_cursor, "glutenFree");
      final int _cursorIndexOfServingSize = CursorUtil.getColumnIndex(_cursor, "servingSize");
      final int _cursorIndexOfUnits = CursorUtil.getColumnIndex(_cursor, "Units");
      final int _cursorIndexOfEnergy = CursorUtil.getColumnIndex(_cursor, "energy");
      final int _cursorIndexOfFat = CursorUtil.getColumnIndex(_cursor, "fat");
      final int _cursorIndexOfOfWhichSaturates = CursorUtil.getColumnIndex(_cursor, "ofWhichSaturates");
      final int _cursorIndexOfFiber = CursorUtil.getColumnIndex(_cursor, "fiber");
      final int _cursorIndexOfProtein = CursorUtil.getColumnIndex(_cursor, "protein");
      final int _cursorIndexOfSalt = CursorUtil.getColumnIndex(_cursor, "salt");
      final int _cursorIndexOfCalories = CursorUtil.getColumnIndex(_cursor, "calories");
      final int _cursorIndexOfSugar = CursorUtil.getColumnIndex(_cursor, "sugar");
      while(_cursor.moveToNext()) {
        if (!_cursor.isNull(_itemKeyIndex)) {
          final long _tmpKey = _cursor.getLong(_itemKeyIndex);
          ArrayList<Ingredients> _tmpRelation = _map.get(_tmpKey);
          if (_tmpRelation != null) {
            final Ingredients _item_1;
            final long _tmpIngredientsId;
            if (_cursorIndexOfIngredientsId == -1) {
              _tmpIngredientsId = 0;
            } else {
              _tmpIngredientsId = _cursor.getLong(_cursorIndexOfIngredientsId);
            }
            ...
            final boolean _tmpMicrowaveable;
            if (_cursorIndexOfMicrowaveable == -1) {
              _tmpMicrowaveable = false;
            } else {
              final int _tmp_1;
              _tmp_1 = _cursor.getInt(_cursorIndexOfMicrowaveable);
              _tmpMicrowaveable = _tmp_1 != 0;
            ...
            final Double _tmpServingSize;
            if (_cursorIndexOfServingSize == -1) {
              _tmpServingSize = null;
            } else {
              if (_cursor.isNull(_cursorIndexOfServingSize)) {
                _tmpServingSize = null;
            ...
            final String _tmpUnits;
            if (_cursorIndexOfUnits == -1) {
              _tmpUnits = null;
            } else {
              _tmpUnits = _cursor.getString(_cursorIndexOfUnits);
            }
            final Double _tmpEnergy;
            if (_cursorIndexOfEnergy == -1) {
              _tmpEnergy = null;
            } else {
              if (_cursor.isNull(_cursorIndexOfEnergy)) {
                _tmpEnergy = null;
            ...
            final Double _tmpOfWhichSaturates;
            if (_cursorIndexOfOfWhichSaturates == -1) {
              _tmpOfWhichSaturates = null;
            } else {
              if (_cursor.isNull(_cursorIndexOfOfWhichSaturates)) {
                _tmpOfWhichSaturates = null;
            ...
            _item_1 = new Ingredients(_tmpIngredientsId,_tmpName,_tmpDescription,_tmpBrand,_tmpServingSize,_tmpUnits,_tmpCalories,_tmpEnergy,_tmpFat,_tmpOfWhichSaturates,_tmpFiber,_tmpProtein,_tmpSalt,_tmpSugar,_tmpFreezable,_tmpMicrowaveable,_tmpVegan,_tmpGlutenFree);
            if (_cursorIndexOfSpoonId != -1) {
              if (_cursor.isNull(_cursorIndexOfSpoonId)) {
                _item_1.spoonId = null;
              } else {
                _item_1.spoonId = _cursor.getLong(_cursorIndexOfSpoonId);
              }
            }
            if (_cursorIndexOfAisle != -1) {
              _item_1.aisle = _cursor.getString(_cursorIndexOfAisle);
            }
            if (_cursorIndexOfNameClean != -1) {
              _item_1.nameClean = _cursor.getString(_cursorIndexOfNameClean);
            }
            if (_cursorIndexOfOriginal != -1) {
              _item_1.original = _cursor.getString(_cursorIndexOfOriginal);
            }
            if (_cursorIndexOfConsistency != -1) {
              _item_1.consistency = _cursor.getString(_cursorIndexOfConsistency);
            }
  ...
  private void __fetchRelationshipMethodAscomExampleZestRoomEntitiesMethod(
      final LongSparseArray<ArrayList<Method>> _map) {
    if (_map.isEmpty()) {
      return;
    }
    // check if the size is too big, if so divide;
    if(_map.size() > RoomDatabase.MAX_BIND_PARAMETER_CNT) {
      LongSparseArray<ArrayList<Method>> _tmpInnerMap = new LongSparseArray<ArrayList<Method>>(androidx.room.RoomDatabase.MAX_BIND_PARAMETER_CNT);
      int _tmpIndex = 0;
      int _mapIndex = 0;
      final int _limit = _map.size();
      while(_mapIndex < _limit) {
        ...
        _tmpIndex++;
        if(_tmpIndex == RoomDatabase.MAX_BIND_PARAMETER_CNT) {
          __fetchRelationshipMethodAscomExampleZestRoomEntitiesMethod(_tmpInnerMap);
          _tmpInnerMap = new LongSparseArray<ArrayList<Method>>(RoomDatabase.MAX_BIND_PARAMETER_CNT);
          _tmpIndex = 0;
        }
      }
      if(_tmpIndex > 0) {
    ...
    StringBuilder _stringBuilder = StringUtil.newStringBuilder();
    _stringBuilder.append("SELECT `Method`.`methodId` AS `methodId`,`Method`.`method` AS `method`,_junction.`mealId` FROM `MealMethodJoin` AS _junction INNER JOIN `Method` ON (_junction.`methodId` = `Method`.`methodId`) WHERE _junction.`mealId` IN (");
    final int _inputSize = _map.size();
    StringUtil.appendPlaceholders(_stringBuilder, _inputSize);
    _stringBuilder.append(")");
    final String _sql = _stringBuilder.toString();
    ...
    final Cursor _cursor = DBUtil.query(__db, _stmt, false, null);
    try {
      final int _itemKeyIndex = 2; // _junction.mealId;
      if (_itemKeyIndex == -1) {
        return;
      }
      final int _cursorIndexOfMethodId = CursorUtil.getColumnIndex(_cursor, "methodId");
      final int _cursorIndexOfMethod = CursorUtil.getColumnIndex(_cursor, "method");
      while(_cursor.moveToNext()) {
        if (!_cursor.isNull(_itemKeyIndex)) {
          final long _tmpKey = _cursor.getLong(_itemKeyIndex);
          ArrayList<Method> _tmpRelation = _map.get(_tmpKey);
          if (_tmpRelation != null) {
            final Method _item_1;
            final long _tmpMethodId;
            if (_cursorIndexOfMethodId == -1) {
              _tmpMethodId = 0;
            } else {
              _tmpMethodId = _cursor.getLong(_cursorIndexOfMethodId);
            }
            final String _tmpMethod;
            if (_cursorIndexOfMethod == -1) {
              _tmpMethod = null;
            } else {
              _tmpMethod = _cursor.getString(_cursorIndexOfMethod);
            }
            _item_1 = new Method(_tmpMethodId,_tmpMethod);
            _tmpRelation.add(_item_1);
          }
        }
      }
